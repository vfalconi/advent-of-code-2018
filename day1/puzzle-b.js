...
while (intersection === null) {
	...
	if (results[iteration] === undefined) results.push([]);

	frequency = frequencyChanges.reduce((sum, change) => {
		const newFrequency = sum + change;
		if (intersection === null) {
			results.forEach((row, index) => {
				if (row.includes(newFrequency)) intersection = newFrequency;
			});
		}
		results[iteration].push(newFrequency);
		return newFrequency
	}, frequency);

	iteration++;
}
console.log(`intersection found with frequency ${intersection}`);
